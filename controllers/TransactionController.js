...
class TransactionController {
  static async createTransaction(req, res, next) {
    try {
      ...
      console.log(
        'maskfjaksdfhakljsdfhajkdsfhaljksdfhkajdhsfkjashdfkjahdfjkahdfjhasdfhasdkfhadskfhakdjsfhalksdf'
      );
      if (!isPackageExist) return next({ name: 'NotFound', message: 'Package not found' });
      if (!isPackageExist.available_slots)
        return next({ name: 'BadRequest', message: 'Stocks book is empty' });
      const isPackageBought = await Transaction.findOne({
        where: {
